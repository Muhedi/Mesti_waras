<?php 
	...
	$query_pbl = "INSERT INTO pembelian (id_pembelian, id_supplier, tgl_pembelian, cr_bayar, jth_tempo, total_pembelian, status_byr, tgl_lunas, id_peg) VALUES ('$id_pembelian', '$id_supplier', '$tgl_pembelian', '$cr_bayar', '$jth_tempo', '$total_pembelian', '$status', '$tgl_lunas', '$id_pegawai')";
	...
	for($i = 0; $i < count($_POST["hidden_kdobat"]); $i++) {
		...
		$query_pbldtl = "INSERT INTO detail_pembelian (id_pembelian, kd_obat, exp_obatbeli, hrg_beli, jml_beli, sat_beli, subtotal) VALUES ('$id_pembelian', '$kd_obat', '$exp_obat', '$hrg_beli', '$jml_obat', '$sat_jual', '$subtotal')";
		mysqli_query($conn, $query_pbldtl) or die ($conn->error);

		$query_stok = "SELECT stk_obat, hrgbeli_obat FROM obat WHERE kd_obat = '$kd_obat'";
		...
		$query_estok = "UPDATE obat SET stk_obat='$stok_baru', hrgbeli_obat = '$harga', hrg_obat = '$harga_jual' WHERE kd_obat='$kd_obat'";
		mysqli_query($conn, $query_estok) or die ($conn->error);

		$query_exp = "SELECT stok FROM stok_obat WHERE kd_obat = '$kd_obat' AND tgl_exp = '$exp_obat'";
		...
		if($rows_exp > 0) {
			...
			$query_updstokexp = "UPDATE stok_obat SET stok='$stok_baruexp' WHERE kd_obat='$kd_obat' AND tgl_exp = '$exp_obat'";
			mysqli_query($conn, $query_updstokexp) or die ($conn->error);
		} else {
			$query_stokexp = "INSERT INTO stok_obat (kd_obat, tgl_exp, stok) VALUES ('$kd_obat', '$exp_obat', '$jml_obat')";

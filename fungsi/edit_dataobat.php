<?php 
	...
	for($i=0; $i<$jml_a; $i++) {
		...
		$stok_s = $a_stok[$i];
		$stok = $stok + $stok_s;
		$query_stok = "UPDATE stok_obat SET tgl_exp = '$exp_s', stok = '$stok_s' WHERE id_stok = '$nmr_stok'";
		$sql_stok = mysqli_query($conn, $query_stok) or die ($conn->error);
	}

	$query = "UPDATE obat SET nm_obat='$nama', ktg_obat='$ktg', sat_obat='$satuan', hrg_obat='$harga_jual', hrgbeli_obat='$harga', stk_obat='$stok', bnt_obat='$bentuk', minstk_obat='$min_stok' WHERE kd_obat = '$kode'";

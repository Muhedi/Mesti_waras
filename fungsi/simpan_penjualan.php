<?php 
 ...
 //    }
	...
	$query_pjl = "INSERT INTO penjualan VALUES('$id_penjualan', '$tgl_penjualan', '$total_penjualan', '$tunai', '$kembali', '$id_pegawai')";
	...
	for($i = 0; $i < count($_POST['hidden_kdobat']); $i++) {
		...
		$query_pjldtl = "INSERT INTO detail_penjualan (id_penjualan, kd_obat, hrg_jual, jml_jual, sat_jual, subtotal) VALUES('$id_penjualan', '$kd_obat', '$hrg_jual', '$jml_obat', '$sat_jual', '$subtotal')";
		mysqli_query($conn, $query_pjldtl) or die ($conn->error);

		$query_stok = "SELECT stk_obat FROM obat WHERE kd_obat = '$kd_obat'";
		...
		$query_estok = "UPDATE obat SET stk_obat='$stok_baru' WHERE kd_obat='$kd_obat'";
		mysqli_query($conn, $query_estok) or die ($conn->error);

		$query_stokexp = "SELECT stok FROM stok_obat WHERE kd_obat = '$kd_obat' AND tgl_exp = '$exp_obat'";
		...
		$query_estokexp = "UPDATE stok_obat SET stok='$stok_baruexp' WHERE kd_obat='$kd_obat' AND tgl_exp = '$exp_obat'";

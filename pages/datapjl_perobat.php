	...
	<div class="table-container">
		<table id="penjualan" class="table table-striped display tabel-data">
			...
			<?php 
				...
				$nomor = 1;
				$query_pjlobat = "SELECT nm_obat, detail_penjualan.kd_obat, sat_obat, SUM(detail_penjualan.jml_jual) AS jml_terjual, SUM(detail_penjualan.hrg_jual*detail_penjualan.jml_jual) AS keuntungan FROM detail_penjualan INNER JOIN obat ON detail_penjualan.kd_obat = obat.kd_obat INNER JOIN penjualan ON detail_penjualan.id_penjualan = penjualan.id_penjualan WHERE (penjualan.tgl_penjualan BETWEEN '' AND '$tgl_sekarang') GROUP BY detail_penjualan.kd_obat ORDER BY nm_obat ASC";

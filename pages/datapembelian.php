  ...
  <div class="row">
    ...
    <div class="kotak-data-tab">
      <ul class="nav nav-pills" id="pills-tab" role="tablist">
          <?php 
            $query_lunas = "SELECT * FROM pembelian INNER JOIN pegawai ON pembelian.id_peg = pegawai.id_peg INNER JOIN supplier ON pembelian.id_supplier = supplier.id_supplier WHERE pembelian.status_byr = 'Lunas' ORDER BY supplier.nama_supp ASC";
            $sql_lunas = mysqli_query($conn, $query_lunas) or die ($conn->error);
            $jml_lunas = mysqli_num_rows($sql_lunas);

            $query_utang = "SELECT * FROM pembelian INNER JOIN pegawai ON pembelian.id_peg = pegawai.id_peg INNER JOIN supplier ON pembelian.id_supplier = supplier.id_supplier WHERE pembelian.status_byr = 'Belum Lunas' ORDER BY pembelian.tgl_pembelian ASC";
        ...
        <div class="tab-pane fade show active" id="dpembelian_lunas" role="tabpanel" aria-labelledby="dpembelian_lunas-tab">
            <div class="table-container">
                <table id="example" class="table table-striped display tabel-data">
                    ...
                    <tbody>
                        <?php 
                            $id_detailpenjualan = 1;
                            while($dlunas = mysqli_fetch_array($sql_lunas)) {
                         ?>
                            <tr>
                                <td><?php echo $id_detailpenjualan++; ?></td>
                                <td><?php echo $dlunas['id_pembelian']; ?></td>
                    ...
                    <tbody>
                        <?php 
                         ...
                         ?>
                            <tr>
                                <td><?php echo $id_detailpenjualan++; ?></td>
                                <td><?php echo $dutang['id_pembelian']; ?></td>
                                ...
                                <td class="td-opsi">
                                    <button class="btn-transition btn btn-outline-primary btn-sm" title="detail obat" id="tombol_detail" name="tombol_detail" data-toggle="modal" data-target="#detail_pembelian" data-id_pembelian="<?php echo $dutang['id_pembelian']; ?>" data-tgl_pembelian="<?php echo tgl_indo($dutang['tgl_pembelian']); ?>" data-nama_supp="<?php echo $dutang['nama_supp']; ?>" data-status_byr="<?php echo $dutang['status_byr']; ?>">
                                        <i class="fas fa-info-circle"></i>
                                    </button>
                                    <!-- <a href="laporan/?page=nota_pembelian&no_faktur=<?php echo $dutang['id_pembelian']; ?>" target="_blank">
                                      <button class="btn-transition btn btn-outline-dark btn-sm" title="cetak" id="tombol_cetak" name="tombol_cetak">
                                          <i class="fas fa-print"></i>
                                      </button>
                                    </a>-->
                                    <button class="btn-transition btn btn-outline-success btn-sm" title="pelunasan" id="tombol_pelunasan" name="tombol_pelunasan" data-id_pembelian="<?php echo $dutang['id_pembelian']; ?>" data-nama_supp="<?php echo $dutang['nama_supp']; ?>">
                                        <i class="fas fa-check-square"></i>
                                    </button>
                                    <?php if($_SESSION['posisi_peg'] == 'Admin' || $_SESSION['posisi_peg'] == 'Manager' || $_SESSION['posisi_peg'] == 'Apoteker') { ?>
                                    <button class="btn-transition btn btn-outline-danger btn-sm" title="hapus" id="tombol_hapus" name="tombol_hapus" data-id_pembelian="<?php echo $dutang['id_pembelian']; ?>">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                    <?php } ?>
                                </td>
...
<script>
  $("button[name='tombol_detail']").click(function() {
    ...
    $("#no_fakturdetail").html(id_pembelian);
    ...
    $.ajax({
      type: "GET",
      url: "fungsi/detail.php?page=pembelian",
  ...
  $("button[name='tombol_hapus']").click(function() {
    ...
    Swal.fire({
          ...
          text: 'anda akan menghapus data pembelian '+id_pembelian+', anda tidak dapat mengembalikan data yang telah dihapus.',
          type: 'warning',
          ...
          if (hapus.value) {
            $.ajax({
              type: "POST",
              url: "fungsi/hapus.php?page=datapembelian",
    ...
    $("button[name='tombol_pelunasan']").click(function() {
        ...
        Swal.fire({
          title: 'Apakah Anda Yakin?',
          text: 'anda telah melunasi pembelian '+id_pembelian+' dengan pihak '+nama_supp+', data ini tidak dapat dirubah kembali.',
          type: 'warning',
          showCancelButton: true,
          cancelButtonColor: '#d33',
          ...
          if (lunas.value) {
            $.ajax({
              type: "POST",
              url: "fungsi/detail.php?page=pelunasan_pembelian",
              data: "id_pembelian="+id_pembelian,

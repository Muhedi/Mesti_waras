	...
	<div class="form-container">
		<div class="row">
			<div class="col-md-6 offset-md-3 offset-form">
				...
				<?php 
				  	$query_tampil = "SELECT * FROM obat WHERE kd_obat='$kd_obat'";
				...
				<form action="javascrip:void" autocomplete="off" id="form_editobat">
				  ...
				  <?php 
				  	$query_stok = "SELECT * FROM stok_obat WHERE kd_obat = '$kd_obat'";
				  	$sql_stok = mysqli_query($conn, $query_stok) or die ($conn->error);
				  	$id_detailpenjualan=1;
				  	while($data_stok = mysqli_fetch_array($sql_stok)) {
				   ?>
				  <div class="form-group row">
				    <label for="ip_kadaluarsa<?php echo $id_detailpenjualan; ?>" class="col-sm-3 col-form-label">Kadaluarsa</label>
				    <div class="col-sm-4">
					    <input type="date" class="form-control form-control-sm" id="ip_kadaluarsa<?php echo $id_detailpenjualan; ?>" name="ip_kadaluarsa[]" value="<?php echo $data_stok['tgl_exp'] ?>">
					    <input type="hidden" id="ip_nmrstok<?php echo $id_detailpenjualan; ?>" name="ip_nmrstok[]" value="<?php echo $data_stok['id_stok'] ?>">
					</div>
					<label for="ip_stok<?php echo $id_detailpenjualan; ?>" class="col-sm-1 col-form-label" style="text-align: right;">Stok</label>
				    <div class="col-sm-4">
					    <input type="number" class="form-control form-control-sm" id="ip_stok<?php echo $id_detailpenjualan; ?>" name="ip_stok[]" value="<?php echo $data_stok['stok'] ?>">
					</div>
				  </div>
				  <?php $id_detailpenjualan++; } ?>
				  <div class="form-group row">

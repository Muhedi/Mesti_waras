			...
			<div class="col-md-6 offset-md-3 offset-form">
				...
				<form action="javascrip:void(0);"  autocomplete="off">
				  <div class="form-group row pt-3">
				    <label for="id_supplier" class="col-sm-3 col-form-label">No Supplier</label>
				    <div class="col-sm-9">
				      <input type="text" class="form-control form-control-sm" id="id_supplier" placeholder="masukkan nama supplier" value="<?php echo $id_supplier; ?>" disabled="">
				    </div>
				  </div>
				  <?php 
				  	$query_tampil = "SELECT * FROM supplier WHERE id_supplier='$id_supplier'";
		...
		if(nama_supp=="") {
			...
			Swal.fire({
	          ...
	          if (ya.value) {
	            $.ajax({
	              type: "POST",
	              url: "fungsi/edit_supplier.php",
	              data: "nama_supp="+nama_supp+"&nama_pet="+nama_pet+"&no_petugas="+no_petugas+"&alm_supp="+alm_supp+"&id_supplier="+id_supplier,
